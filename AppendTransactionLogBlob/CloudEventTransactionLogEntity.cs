...
namespace AppendTransactionLogBlob
{
    class CloudEventTransactionLogEntity : TableEntity
    {
        ...
        {
            var tranEnd2 = new CloudEventLogSequenceNumber(tranEnd.Log, tranEnd.SequenceNumber - 1); // see GetEnd

            PartitionKey = CloudEventTransaction.PartitionKey;
            RowKey = CloudEventTransaction.RowKeyTransactionPrefix + tranEnd.LogFixed8 + "-" + tranEnd2.SequenceNumberFixed12;
            LSN = tranStart.Value;
            IDX = idx;
        }
